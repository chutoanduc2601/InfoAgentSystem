...
import { MessageSquarePlus, Loader2, AlertTriangle, ArrowLeft } from 'lucide-react';
    ...
    return (
      <div className="flex h-full w-full items-center justify-center bg-white p-20">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-800">Đang phân tích...</h2>
    ...
    return (
      <div className="flex h-full w-full items-center justify-center bg-white">
        <div className="text-center max-w-md">
          ...
          <Link to="/" className="px-6 py-3 bg-blue-600 text-white rounded-xl">Thử lại</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full w-full overflow-hidden bg-white">
      <div className="flex-1 overflow-y-auto px-8 py-10">
        <ReportHeader title={report.query} status={report.confidence_label} timestamp={new Date().toLocaleString()} />
          ...
          <div className="mt-8">
            ...
            <div className="flex flex-wrap gap-2">
              {report.recommendations.map((r: string, i: number) => (
                <span key={i} className="px-3 py-1 bg-blue-50 text-blue-700 rounded-lg text-sm">{r}</span>
              ))}
            </div>
          </div>
        )}
